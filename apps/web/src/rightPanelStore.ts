...
export type RightPanelSurface =
  ...
  | { id: "current-tasks"; kind: "current-tasks" }
  | { id: "rtx-orchestrator"; kind: "rtx-orchestrator" };

const RIGHT_PANEL_STORAGE_KEY = "t3code:right-panel-state:v2";
// v9 removed the "plan" surface kind (plans render inline in the transcript).
// v10 keys pull-request surfaces by reference instead of a singleton tab.
// v11 stops persisting the pull-request list's shared panel, so a restart opens the page fresh.
// v12 adds the RedClaw-backed Current Tasks and RTX Orchestrator surfaces.
...
const singletonSurface = (
  ...
  switch (kind) {
    ...
    case "current-tasks":
      return { id: "current-tasks", kind };
    case "rtx-orchestrator":
      return { id: "rtx-orchestrator", kind };
  }
};
