...
export const RedClawSettings = Schema.Struct({
  // Runtime registration is additionally gated by validated server-only
  // environment configuration. No credentials are persisted in settings.
  enabled: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  customModels: Schema.Array(Schema.String).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
});
export type RedClawSettings = typeof RedClawSettings.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    redclaw: RedClawSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
  }).pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const RedClawModelOptionsPatch = Schema.Struct({
  ...RedClawModelOptions.fields,
});
...
const ModelSelectionPatch = Schema.Union([
  ...
  Schema.Struct({
    provider: Schema.optionalKey(Schema.Literal("redclaw")),
    model: Schema.optionalKey(TrimmedNonEmptyString),
    options: Schema.optionalKey(RedClawModelOptionsPatch),
  }),
]);
...
const RedClawSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      redclaw: Schema.optionalKey(RedClawSettingsPatch),
    }),
  ),
});
